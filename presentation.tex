...
\begin{frame}
\begin{exampleblock}{[Projet] Step 1 - Synchronisation 1}
\begin{itemize}
...
\begin{frame}
\begin{exampleblock}{[GIT] Exercice \Roman{exampleBlockCounter} - Initialisation du dépôt local}\stepcounter{exampleBlockCounter} 
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item Ajouter les fichiers à l'index : commande "git add"
...
\begin{frame}
\begin{exampleblock}{[PROJET] Step 2 - Synchronisation 2}
\begin{itemize}
...
\begin{exampleblock}{[GIT] Exercice \Roman{exampleBlockCounter} - Step 2 du projet}\stepcounter{exampleBlockCounter} 
\begin{enumerate}
    \item Ajouter les fichiers à l'index : commande "git add"
    \item Archiver cette étape : commande "git commit"
    \item Visualisez l'historique avec "git log" 
\end{enumerate}
\end{exampleblock}
\end{frame}
...
\end{tikzpicture}
\begin{itemize}
	\item \textbf{ad6c0bb} est l'ID du \textbf{commit} (l'ID complet est un SHA1)
	\item \textbf{main} (ou \textbf{master}) est une \textbf{référence} au commit
	\item \textbf{HEAD} est une autre référence : celle de la "tête de lecture"
\end{itemize}
\end{block}
\end{frame}
...
\end{tikzpicture}
\begin{itemize}
	\item Il y a maintenant deux \textbf{commits}
	\item La \textbf{référence main} pointera toujours sur le dernier \textbf{commit}
	\item On voit apparaitre une \textbf{branche} qui comporte ces deux \textbf{commits}
	\item Cette \textbf{branche} porte le même nom que la \textbf{référence} qui pointe sur son dernier \textbf{commit} : \textbf{branche main}
	\item La nom \textbf{main} est par convention utilisé pour la \textbf{branche} de production, la \textbf{branche} principale
...
\begin{frame}
\begin{exampleblock}{[Projet] Step 3 - Comptage temporel}
\begin{itemize}
...
\begin{exampleblock}{[GIT] Exercice \Roman{exampleBlockCounter} - Step 3 du projet}\stepcounter{exampleBlockCounter} 
\begin{enumerate}
    \item Ajouter les fichiers à l'index : commande "git add"
    \item Archiver cette.
     étape : commande "git commit"
     \item Visualisez le résumé de l'historique avec "git log -{}-oneline" 
\end{enumerate}
\end{exampleblock}
\end{frame}

\begin{frame}
\begin{block}{État après le troisième commit local}
\centering
\begin{tikzpicture}
	\gitDAG[grow right sep = 2em]{ad6c0bb -- 55e023d -- 84df6e0};
    \gitremotebranch
    	[main]    % node name
    	{main} % node text
        {right=of 84df6e0}    % node placement
        {84df6e0}             % target
     \gitHEAD
        {above=of main} % node placement
        {main}  
\end{tikzpicture}
\begin{itemize}
	\item La \textbf{branche main} comporte maintenant un \textbf{commit} supplémentaire
	\item La \textbf{référence main} pointe donc toujours sur le dernier \textbf{commit} de sa \textbf{branche}, on ne peut pas la déplacer
\end{itemize}
\end{block}
\end{frame}

\begin{frame}
\begin{exampleblock}{[Projet] Step 4 - Transmission du comptage temporel}
\begin{itemize}
...
\begin{exampleblock}{[GIT] Exercice \Roman{exampleBlockCounter} - Step 4 du projet}\stepcounter{exampleBlockCounter} 
\begin{enumerate}
    \item Ajouter les fichiers à l'index : commande "git add"
    \item Archiver cette.
     étape : commande "git commit"
...
\begin{frame}
\begin{block}{État après le quatrième commit local}
\centering
\begin{tikzpicture}
	\gitDAG[grow right sep = 2em]{ad6c0 -- 55e02 -- 84df6 -- 7841f};
    \gitremotebranch
    	[main]    % node name
    	{main} % node text
        {right=of 7841f}    % node placement
        {7841f}             % target
     \gitHEAD
        {above=of main} % node placement
        {main}  
\end{tikzpicture}
\begin{itemize}
	\item La \textbf{branche main} comporte un autre \textbf{commit}
	\item La \textbf{référence main} pointe toujours sur le dernier \textbf{commit}
	\item La \textbf{référence HEAD}, la "tête de lecture", peut être déplacée pour explorer l'historique du dépôt
\end{itemize}
\end{block}
\begin{exampleblock}{[GIT] Exercice \Roman{exampleBlockCounter} - Step 4 du projet : se déplacer dans l'historique}\stepcounter{exampleBlockCounter} 
\begin{enumerate}
    \item Utiliser la commande "git checkout 55e02" pour revenir à l'étade 2 du projet
    \item Utiliser la commande "git checkout main" pour revenir en tête de la branche main
\end{enumerate}
\end{exampleblock}
\end{frame}

\begin{frame}
\begin{block}{État après la commande "git checkout 55e02"}
\centering
\begin{tikzpicture}
	\gitDAG[grow right sep = 2em]{ad6c0 -- 55e02 -- 84df6 -- 7841f};
    \gitremotebranch
        ...
        {7841f}             % target
     \gitHEAD
        {above=of 55e02} % node placement
        {55e02} 			
\end{tikzpicture}
\begin{itemize}
	\item La \textbf{référence HEAD} est déplacée au deuxième \textbf{commit}
	\item Les fichiers archivés se retrouvent alors dans l'état où ils étaient lors de la deuxième étape du projet
	\item On peut inspecter et même faire des changements
	\item Ils seront perdus si on déplace de nouveau \textbf{HEAD}
	\item Pour prendre en compte les changements, il faut créer une nouvelle \textbf{branche} et y archiver (\textbf{commit}) ces changements 
\end{itemize}
\end{block}
\end{frame}

\begin{frame}
\begin{block}{État après la commande "git checkout main"}
\centering
\begin{tikzpicture}
	\gitDAG[grow right sep = 2em]{ad6c0 -- 55e02 -- 84df6 -- 7841f};
    \gitremotebranch
...
\end{tikzpicture}
\begin{itemize}
	\item La \textbf{référence HEAD} pointe de nouveau sur le dernier \textbf{commit} de la \textbf{branche main}
	\item Les fichiers archivés se retrouvent alors dans l'état le plus récent de la \textbf{branche main} et on peut continuer le travail 
\end{itemize}
\end{block}
\end{frame}
...
\begin{exampleblock}{[Projet] Step 5 - Transmission de la valeur courante du capteur}
\begin{itemize}
\item Lire la valeur brute du capteur (cf. doc Projets)
\item Transmettre la valeur au PC
\item Vous utiliserez les mêmes fonctions sprintf() et Serial.println()
\end{itemize}
\end{exampleblock}

\justifying
\textcolor{red}{\textbf{Interruption inattendue : vous n'avez pas terminé cette étape et vous devez faire des modifications qui doivent être utilisées immédiatement sur le poste expérimental : changer les caractères 'r' et 's' par 'd' et 'a' pour le démarrage et l'arrêt d'un essai ! Vous ne voulez pas perdre les modifs en cours mais vous ne pouvez pas les utiliser immédiatement puisque vous n'avez pas terminé l'étape 5 ! ...}}
\end{frame}

\begin{frame}
\justifying
Une bonne pratique de git pourrait se résumer ainsi : "Il est souvent très utile, surtout quand un code est en production, d'utiliser une nouvelle branche lorsqu'on le fait évoluer !". Dans notre contexte, un code en production peut se traduire par "un code en cours d'utilisation dans le cadre d'une expérience, que l'on soit en phase d'acquisition ou de traitement des données". 
\vspace{1cm}

\centering
Vous allez suivre cette bonne pratique !
\end{frame}

\begin{frame}

\begin{exampleblock}{[GIT] Exercice \Roman{exampleBlockCounter} - Step 5 du projet : création d'une branche}
\stepcounter{exampleBlockCounter}
\begin{enumerate}
    \item Créez une branche nommée "step5"
    \item Archivez les modifs en cours dans cette branche
    \item Retournez dans la branche main et faites les changements de caractères
    \item Archivez ces changements et utilisez-les en poste !
    \item Retournez dans la branche "step5" et terminez l'étape
\end{enumerate}
\end{exampleblock}

\begin{exampleblock}{[GIT] Exercice \Roman{exampleBlockCounter} -  Step 5 du projet : fusion de branches}
\stepcounter{exampleBlockCounter}
Pour appliquer les améliorations de l'étape 5 dans la branche principale (main) :
\begin{enumerate}
    \item Vous retournez dans la branche main
    \item Vous fusionnez la branche "step5" dans cette branche
\end{enumerate}
\end{exampleblock}

\end{frame}

\begin{frame}{Petit résumé intermédiaire}
...
$ git merge bName        # Merge bName in current branch
    \end{lstlisting}
\end{alertblock}
...
\begin{frame}
    \begin{block}{Centralized VCS - Avant... un peux moins longtemps ! 90's}
     \begin{itemize}
...
\begin{frame}
    \begin{block}{Distributed VCS - Maintenant, depuis un moment (début 2000) !}
     \begin{itemize}
            ...
            \item Travail privé possible, sans serveur
        \end{itemize}
    \centering
